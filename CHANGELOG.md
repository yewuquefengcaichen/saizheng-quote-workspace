...
- 首页上传报价后，改成 **直接跳 `/quotes` 报价台**
- 列映射确认改成 **报价台内嵌确认面板**
- 不再先在首页弹大悬浮模态打断业务流
- 报价台左列补独立滚动，鼠标停留左列即可滚左列
- 报价台候选商品图改成首屏优先加载，减少“看不到图”的误判
- 首页、报价台、商品库、模板页、历史页、词库页继续压缩无关说明文字
- 导出弹层、列映射区、报价台状态块和商品卡片文案继续收短
- 商品图片前端优先本地代理图，减少外链失效导致的空图
- 历史页增加轻量“数据源”状态，当前可直接看到 PostgreSQL V2 / SQLite Legacy
- 新增 `docs/PROJECT_PROGRESS.md`，明确当前处于哪个里程碑以及下一次阶段停止点
- 新增 `backend/scripts/generate_image_embeddings_batch.py`，支持全量 CLIP 向量分批慢跑
- 静态资源版本号提升到 `20260425b`，降低旧缓存干扰

### 当前效果

- 业务页更接近“左边小工作块 + 右边候选选品”的目标
- 首页上传报价后的路径更短：上传 → 进报价台 → 列映射 → 候选确认
- 报价台不再依赖首页大模态完成映射确认

### 下一步

1. README / 任务清单 / 回退说明继续与当前行为对齐
2. 图搜图第二阶段：全量 CLIP 向量生成、缓存、反馈精排
3. PostgreSQL 收口：历史页、词库、模板规则逐步摆脱 JSON / SQLite 主读写
...
## 2026-04-26 - 数据收口与图搜图继续推进

### 已完成

- 历史页 API 继续以 PostgreSQL V2 为主读源，历史页前端已能直接识别 `postgres_v2` 数据源
- 词库已切到 PostgreSQL 主读写，JSON 保留为双写回退位
- 模板已切到 PostgreSQL 主读写，JSON 保留为双写回退位
- FastAPI 图搜图 `POST /api/v1/image-search/query` 已接入 Redis 查询缓存第一版
- 图搜图缓存 key 已按图片 bytes sha256 + provider / model / top_k / query 条件生成，TTL 为 6 小时
- Redis 不可用时图搜图会自动降级，不会影响主流程返回
- 图搜图服务已接入历史反馈精排第一版：按 `MatchFeedback.action='select'` 对命中的候选商品做保守加分
- CLIP 全量向量已完成：`clip_local / openclip_vit_b_32_512d = 3300 / 3300`，`pending_assets = 0`
- 报价台恢复为连续下滑找候选主模式，不再强制单焦点逐项切换
- 报价台以图识图默认切到 `智能 CLIP`，`近似图` 降级为辅助模式，并修正图搜图候选召回/重排过早截断问题
- README、项目进度、升级任务清单、Release 草稿已同步到当前冻结口径

### 当前阶段结论

- 里程碑 1（稳定工作台）已完成
- 里程碑 2（数据收口）已拿下三个关键模块：**历史页 + 词库 + 模板**
- 里程碑 3（图搜图生产化）已拿下：**Redis 缓存第一版 + 反馈精排第一版 + CLIP 全量完成**
- 当前阶段已进入：**v0.9.1 上线前最后收口**

### 下一步

1. 做核心回归：报价台 / 历史 / 词库 / 模板 / 图搜图
2. 完成文档齐套、版本冻结、commit / tag / push
3. 完成上线准备与最终检查
